        ...
        <div className="fixed inset-0 z-[75] flex items-end md:items-center justify-center sm:p-4">
            ...
            <div className="relative bg-[#F5F5F7] w-full md:max-w-lg h-[85vh] md:h-[80vh] rounded-t-3xl md:rounded-3xl shadow-2xl flex flex-col animate-slide-up md:animate-fade-in overflow-hidden">
                <div className="bg-white px-6 py-4 flex items-center justify-between border-b border-gray-100 shadow-sm z-10">
                    <div className="flex items-center gap-3"><div className="p-2 bg-black rounded-full text-white"><Bot className="h-5 w-5" /></div><div><h3 className="font-bold text-gray-900">哭PC AI配單系統</h3><p className="text-[10px] text-green-500 font-bold flex items-center gap-1"><span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" /> 線上</p></div></div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    <div className="flex items-start gap-3"><div className="w-8 h-8 bg-black rounded-full flex items-center justify-center text-white flex-shrink-0 mt-1"><Bot className="h-4 w-4" /></div><div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm max-w-[85%] text-sm leading-relaxed text-gray-700 border border-gray-100">嗨！我是您的 AI 組裝顧問。<br/>請告訴我您的<b>預算</b>以及<b>主要用途</b>，我將為您推薦最適合的配置。</div></div>
                    {(aiBudget || aiUsage) && !aiExplanation && (<div className="flex justify-end"><div className="bg-blue-600 text-white p-4 rounded-2xl rounded-tr-none shadow-sm max-w-[85%] text-sm">預算: ${aiBudget || '?'}<br/>用途: {aiUsage || '?'}</div></div>)}
                    ...
                    {aiExplanation && (<div className="flex items-start gap-3 animate-fade-in"><div className="w-8 h-8 bg-black rounded-full flex items-center justify-center text-white flex-shrink-0 mt-1"><Check className="h-4 w-4" /></div><div className="space-y-2 max-w-[85%]"><div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm text-sm leading-relaxed text-gray-700 border border-gray-100 whitespace-pre-line">{aiExplanation}</div><div className="flex gap-2"><button onClick={() => { setIsAiModalOpen(false); setAiExplanation(''); }} className="px-4 py-2 bg-black text-white text-xs font-bold rounded-xl shadow-md active:scale-95 transition-transform">採用此配置</button><button onClick={() => setAiExplanation('')} className="px-4 py-2 bg-white text-gray-600 text-xs font-bold rounded-xl border border-gray-200 active:scale-95 transition-transform">重新詢問</button></div></div></div>)}
                </div>
                {!aiExplanation && (<div className="p-4 bg-white border-t border-gray-100"><div className="flex flex-col gap-3"><div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">{usagePresets.map(preset => (<button key={preset.label} onClick={() => setAiUsage(preset.value)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${aiUsage === preset.value ? 'bg-black text-white border-black' : 'bg-gray-100 text-gray-600 border-transparent'}`}>{preset.label}</button>))}</div><div className="flex gap-2"><input type="number" value={aiBudget} onChange={(e) => setAiBudget(e.target.value)} placeholder="預算..." className="w-1/3 p-3 bg-gray-100 rounded-xl text-sm font-bold focus:bg-white focus:ring-2 focus:ring-black outline-none transition-all" /><input type="text" value={aiUsage} onChange={(e) => setAiUsage(e.target.value)} placeholder="描述您的需求..." className="flex-1 p-3 bg-gray-100 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-black outline-none transition-all" /><button onClick={handleAiAutoBuild} disabled={isAiLoading || !aiBudget} className="p-3 bg-blue-600 text-white rounded-xl active:scale-90 transition-transform disabled:opacity-50 shadow-lg shadow-blue-200"><Send className="h-5 w-5" /></button></div></div></div>)}
            </div>
        </div>
      )}
      ...
      {isTemplateModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
              ...
              <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl p-6 animate-fade-in">
                  <div className="flex justify-between items-center mb-6"><div className="flex items-center gap-3"><div className="p-2.5 bg-gray-100 rounded-xl">{templateMode === 'save' ? <Save className="h-6 w-6 text-black" /> : <FolderOpen className="h-6 w-6 text-black" />}</div><div><h3 className="text-xl font-bold text-gray-900">{templateMode === 'save' ? '儲存配置' : '載入配置'}</h3><p className="text-xs text-gray-500">{templateMode === 'save' ? '將目前的清單儲存為範本' : '從儲存的範本中還原'}</p></div></div><button onClick={() => setIsTemplateModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full text-gray-400 hover:text-black"><X className="h-5 w-5" /></button></div>
                  {templateMode === 'save' ? (<div className="space-y-4"><div><label className="block text-sm font-bold text-gray-700 mb-2">範本名稱</label><input type="text" value={newTemplateName} onChange={(e) => setNewTemplateName(e.target.value)} placeholder="例如: 遊戲機 40K (2024)" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-black focus:border-transparent font-bold transition-all" autoFocus /></div><button onClick={handleSaveTemplate} className="w-full py-3.5 bg-black text-white rounded-xl font-bold hover:bg-gray-800 transition-colors shadow-lg active:scale-95">確認儲存</button></div>) : (<div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1 custom-scrollbar">{templates.length === 0 ? (<div className="text-center text-gray-400 py-10 bg-gray-50 rounded-2xl border border-dashed border-gray-200"><FolderOpen className="h-10 w-10 mx-auto mb-2 opacity-20" /><p className="text-sm">暫無儲存的範本</p></div>) : (templates.map(template => (<div key={template.id} className="group border border-gray-200 rounded-xl p-4 hover:border-black transition-all bg-white hover:shadow-md cursor-pointer" onClick={() => handleLoadTemplate(template)}><div className="flex justify-between items-start mb-2"><div><div className="font-bold text-gray-900 text-lg group-hover:text-blue-600 transition-colors">{template.name}</div><div className="text-xs text-gray-400 flex items-center gap-1 mt-0.5"><Clock className="h-3 w-3" /> {new Date(template.timestamp).toLocaleString()}</div></div><button onClick={(e) => { e.stopPropagation(); handleDeleteTemplate(template.id); }} className="text-gray-400 hover:text-red-500 p-2 rounded-lg hover:bg-red-50 transition-colors"><Trash2 className="h-4 w-4" /></button></div><div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50"><span className="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded">{template.items.length} 個零件</span><span className="text-xs font-bold text-blue-600">點擊載入</span></div></div>)))}</div>)}
              </div>
          </div>
      )}
