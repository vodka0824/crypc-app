...
import { X, Search, RefreshCw, Plus, Minus, Trash2, ListFilter, ChevronDown, ChevronRight, Check, ArrowUp, ArrowDown, ArrowUpDown, Box } from 'lucide-react';
...
const ProductSelectionModal: React.FC<ProductSelectionModalProps> = ({
    ...
    const ProductRow = ({ index, style, width }: { index: number; style: React.CSSProperties; width: number }) => {
        // Determine column count based on container width
        const columnCount = width >= 1280 ? 3 : width >= 768 ? 2 : 1;
        const startIndex = index * columnCount;
        const items = filteredModalProducts.slice(startIndex, startIndex + columnCount);

        return (
            <div style={style} className="flex gap-4 px-4 md:px-6">
                {items.map((product) => {
                    ...
                    return (
                        <div key={product.id} className={`flex-1 min-w-0 group bg-white md:border md:border-gray-100 md:rounded-2xl p-3 md:p-4 hover:shadow-xl hover:border-gray-200 transition-all border-b border-gray-100 last:border-0 md:border-b ${isSelected ? 'md:bg-blue-50/20 bg-blue-50/20 border-b-gray-200' : ''}`}>
                            <div className={`flex flex-row md:flex-col gap-3 md:gap-4 items-center md:items-stretch h-full`}>
                                ...
                                <div className="flex-1 min-w-0 flex flex-col md:block justify-center h-full">
                                    <div className="flex justify-between items-start mb-0.5 md:mb-1">
                                        <h4 className={`font-bold leading-tight text-sm line-clamp-2 ${isSelected ? 'text-black' : 'text-gray-900'}`} title={product.name}>{product.name}</h4>
                                    </div>
                ...
                {Array.from({ length: columnCount - items.length }).map((_, i) => (
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center sm:p-4">
            ...
            <div className="relative bg-white w-full md:max-w-7xl h-[92vh] md:h-[90vh] rounded-t-3xl md:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-slide-up md:animate-scale-in border border-white/50">
                
                {/* Mobile Drag Handle */}
                <div className="md:hidden w-full flex justify-center pt-3 pb-1" onClick={onClose}><div className="w-12 h-1.5 bg-gray-200 rounded-full" /></div>
                
                {/* Modal Header */}
                <div className="hidden md:flex px-8 py-5 border-b border-gray-100 justify-between items-center bg-white z-10 flex-shrink-0">
                    <div>
                         <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
                            {replacingItemId ? <RefreshCw className="h-6 w-6 text-blue-600" /> : <Plus className="h-6 w-6" />}
                ...
                <div className="md:hidden px-4 pb-3 border-b border-gray-100 flex flex-col gap-3 bg-white sticky top-0 z-20">
                    <div className="flex gap-2 items-center">
                        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="搜尋商品..." className="w-full pl-9 pr-8 py-2.5 bg-gray-100 border-transparent focus:bg-white focus:border-black focus:ring-1 rounded-xl text-sm transition-all outline-none font-medium" />{searchQuery && (<button onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 bg-gray-200 rounded-full text-gray-500"><X className="h-3 w-3" /></button>)}</div><button onClick={onClose} className="p-2.5 bg-gray-100 rounded-xl text-gray-500"><X className="h-5 w-5" /></button>
                    </div>
                    <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">{categoryFilters[activeCategory]?.map(filter => { const isActive = Object.keys(activeFilters).includes(filter.key as string); return (<button key={filter.key} onClick={() => setMobileFiltersOpen(true)} className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors flex items-center gap-1 ${isActive ? 'bg-black text-white border-black' : 'bg-white text-gray-600 border-gray-200'}`}>{filter.label} <ChevronDown className="h-3 w-3" /></button>) })}</div>
                </div>

                <div className="flex flex-1 min-h-0">
                    {/* Sidebar Filters */}
                    <div className="hidden lg:block w-52 border-r border-gray-100 bg-gray-50 overflow-y-auto custom-scrollbar flex-shrink-0">
                        <div className="p-4">
                            ...
                            <div className="space-y-1">{categoryFilters[activeCategory]?.map(filter => { const options = getSmartOptions(allProducts, activeCategory, filter.key, searchQuery, activeFilters); if (options.length === 0) return null; const isExpanded = expandedNodes[filter.key] ?? true; const activeCount = activeFilters[filter.key]?.length || 0; return (<div key={filter.key} className="border-b border-gray-200 last:border-0 pb-3 mb-1"><button onClick={() => toggleNode(filter.key as string)} className="w-full flex items-center justify-between py-2 text-left group transition-colors rounded-lg hover:bg-gray-100 px-2 -mx-2"><div className="flex items-center gap-2"><h4 className={`font-bold text-sm ${activeCount > 0 ? 'text-black' : 'text-gray-700'} group-hover:text-black`}>{filter.label}</h4>{activeCount > 0 && (<span className="bg-black text-white text-[10px] px-1.5 py-0.5 rounded-full ml-auto">{activeCount}</span>)}{isExpanded ? <ChevronDown className="h-4 w-4 text-gray-400" /> : <ChevronRight className="h-4 w-4 text-gray-400" />}</div></button>{isExpanded && (<div className="pl-1 mt-1 space-y-1 mb-3">{options.map((option: string) => { const isChecked = activeFilters[filter.key]?.includes(option); return (<button key={option} onClick={() => toggleFilter(filter.key as string, option)} className={`w-full text-left px-3 py-1.5 rounded-lg text-sm transition-all flex items-center gap-2 group/opt ${isChecked ? 'bg-black text-white font-bold' : 'text-gray-600 hover:bg-gray-200'}`}><div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${isChecked ? 'border-white bg-black' : 'border-gray-300 bg-white group-hover/opt:border-gray-400'}`}>{isChecked && <Check className="h-3 w-3 text-white" />}</div><span className="truncate">{option}</span></button>); })}</div>)}</div>); })}</div></div>
                    </div>
                    
                    <div className="flex-1 flex flex-col min-w-0 bg-white relative">
                         {/* Search & Sort Bar (Desktop) */}
                         <div className="hidden lg:flex px-6 py-4 border-b border-gray-100 bg-white/80 justify-between items-center text-sm sticky top-0 z-20 backdrop-blur-md">
                            <div className="relative w-72">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-9 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-black focus:ring-1 focus:ring-black outline-none transition-all font-medium" 
                                />
                                {searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-black p-1 hover:bg-gray-200 rounded-full"><X className="h-3 w-3" /></button>}
                            </div>
                            <div className="flex items-center gap-6">
                                <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">排序方式:</span>
                                <div className="font-bold text-gray-600 hover:text-black cursor-pointer flex items-center gap-1 transition-colors select-none group" onClick={() => setModalSort(prev => { if (prev === 'name-asc') return 'name-desc'; if (prev === 'name-desc') return 'default'; return 'name-asc'; })}>
                                    名稱 {modalSort === 'name-asc' && <ArrowUp className="h-3.5 w-3.5 text-black" />}{modalSort === 'name-desc' && <ArrowDown className="h-3.5 w-3.5 text-black" />}{modalSort !== 'name-asc' && modalSort !== 'name-desc' && <ArrowUpDown className="h-3.5 w-3.5 text-gray-400 group-hover:text-gray-600" />}
                                </div>
                                <div className="font-bold text-gray-600 hover:text-black cursor-pointer flex items-center justify-end gap-1 transition-colors select-none group min-w-[80px]" onClick={() => setModalSort(prev => { if (prev === 'price-asc') return 'price-desc'; if (prev === 'price-desc') return 'default'; return 'price-asc'; })}>
                                    金額 {modalSort === 'price-asc' && <ArrowUp className="h-3.5 w-3.5 text-black" />}{modalSort === 'price-desc' && <ArrowDown className="h-3.5 w-3.5 text-black" />}{modalSort !== 'price-asc' && modalSort !== 'price-desc' && <ArrowUpDown className="h-3.5 w-3.5 text-gray-400 group-hover:text-gray-600" />}
                                </div>
                            </div>
                         </div>
                         ...
                         <div className="flex-1 bg-gray-50/30">
                             {filteredModalProducts.length === 0 ? (
                                 <div className="h-full flex flex-col items-center justify-center text-gray-400">
                                     ...
                                     <button onClick={() => { setActiveFilters({}); setSearchQuery(''); }} className="mt-4 px-6 py-2 bg-black text-white rounded-lg text-sm font-bold shadow-md hover:scale-105 transition-transform">清除所有篩選</button>
                                 </div>
                             ) : (
                                <AutoSizer>
                                    {({ height, width }) => {
                                        const columnCount = width >= 1280 ? 3 : width >= 768 ? 2 : 1;
                                        const rowCount = Math.ceil(filteredModalProducts.length / columnCount);
                                        const itemHeight = width >= 768 ? 320 : 130;
                                        ...
                                        return (
                                            <List
                                                ...
                                                width={width}
                                            >
                                                {({ index, style }) => <ProductRow index={index} style={style} width={width} />}
                                            </List>
                                        );
                                    }}
                                </AutoSizer>
