        ...
        return (
            <div style={style} className="flex gap-4 px-4 md:px-6">
                {items.map((product) => {
                    ...
                    return (
                        <div key={product.id} className={`flex-1 min-w-0 group bg-white md:border md:border-gray-100 md:rounded-2xl p-3 md:p-4 hover:shadow-lg hover:border-gray-200 transition-all border-b border-gray-100 last:border-0 md:border-b ${isSelected ? 'md:bg-white bg-gray-50 border-b-gray-200' : ''}`}>
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center sm:p-4">
            ...
            <div className="relative bg-white w-full md:max-w-7xl h-[92vh] md:h-[90vh] rounded-t-3xl md:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slide-up md:animate-fade-in">
                ...
                <div className="hidden md:flex px-6 py-4 border-b border-gray-100 justify-between items-center bg-white z-10 flex-shrink-0"><div><h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">{replacingItemId ? <RefreshCw className="h-5 w-5 text-blue-600" /> : <Plus className="h-5 w-5" />}{replacingItemId ? '更換' : '選擇'} {categoryDisplayMap[activeCategory]}</h2></div><button onClick={onClose} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200"><X className="h-5 w-5" /></button></div>
                ...
                <div className="flex flex-1 min-h-0">
                    <div className="hidden lg:block w-48 border-r border-gray-100 bg-gray-50 overflow-y-auto custom-scrollbar flex-shrink-0">
                        <div className="p-3"><div className="flex items-center justify-between mb-6"><div className="flex items-center gap-2 text-gray-500 font-bold text-sm uppercase tracking-wider"><ListFilter className="h-4 w-4" /> 篩選條件</div>{Object.keys(activeFilters).length > 0 && (<button onClick={() => setActiveFilters({})} className="text-xs text-blue-600 hover:text-blue-800 font-bold hover:underline">清除全部</button>)}</div><div className="space-y-1">{categoryFilters[activeCategory]?.map(filter => { const options = getSmartOptions(allProducts, activeCategory, filter.key, searchQuery, activeFilters); if (options.length === 0) return null; const isExpanded = expandedNodes[filter.key] ?? true; const activeCount = activeFilters[filter.key]?.length || 0; return (<div key={filter.key} className="border-b border-gray-200 last:border-0 pb-2 mb-2"><button onClick={() => toggleNode(filter.key as string)} className="w-full flex items-center justify-between py-2 text-left group transition-colors rounded-lg hover:bg-gray-100 px-2 -mx-2"><div className="flex items-center gap-2"><h4 className={`font-bold text-sm ${activeCount > 0 ? 'text-black' : 'text-gray-700'} group-hover:text-black`}>{filter.label}</h4>{activeCount > 0 && (<span className="bg-black text-white text-[10px] px-1.5 py-0.5 rounded-full ml-auto">{activeCount}</span>)}{isExpanded ? <ChevronDown className="h-4 w-4 text-gray-400" /> : <ChevronRight className="h-4 w-4 text-gray-400" />}</div></button>{isExpanded && (<div className="pl-1 mt-1 space-y-1 mb-3">{options.map((option: string) => { const isChecked = activeFilters[filter.key]?.includes(option); return (<button key={option} onClick={() => toggleFilter(filter.key as string, option)} className={`w-full text-left px-3 py-1.5 rounded-lg text-sm transition-all flex items-center gap-2 group/opt ${isChecked ? 'bg-black text-white font-bold' : 'text-gray-600 hover:bg-gray-200'}`}><div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${isChecked ? 'border-white bg-black' : 'border-gray-300 bg-white group-hover/opt:border-gray-400'}`}>{isChecked && <Check className="h-3 w-3 text-white" />}</div><span className="truncate">{option}</span></button>); })}</div>)}</div>); })}</div></div>
                    </div>
                             ...
                             {filteredModalProducts.length === 0 ? (
                                ...
                                <AutoSizer>
                                    {({ height, width }) => {
                                        ...
                                        const itemHeight = width >= 768 ? 320 : 130; // Approx height for cards
