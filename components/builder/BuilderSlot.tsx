...
const BuilderSlot: React.FC<BuilderSlotProps> = ({
    ...
    return (
        <div className={`
            bg-white rounded-2xl transition-all duration-300 overflow-hidden group relative
            ${hasItems 
                ? 'shadow-[0_4px_20px_-4px_rgba(0,0,0,0.08)] border border-blue-50/50 z-10' 
                : 'shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] border border-transparent hover:shadow-[0_8px_24px_-4px_rgba(0,0,0,0.1)] hover:border-gray-100 hover:z-10'
            }
        `}>
            {hasItems ? (
                <div className="px-4 py-3 md:px-5 md:py-4 bg-gray-50/30 border-b border-gray-100/50 flex justify-between items-center backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        <div className="p-1.5 md:p-2 rounded-xl bg-black text-white shadow-md shadow-gray-200">
                            <Icon className="h-3.5 w-3.5 md:h-5 md:w-5" />
                        </div>
                        <span className="font-bold text-sm md:text-lg text-gray-800">{label}</span>
                        <span className="text-[10px] md:text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-bold">{items.length}</span>
                    </div>
                    <div className="flex items-center gap-2 md:gap-3">
                        <div className="text-xs md:text-base font-bold text-gray-900 hidden sm:block">
                            <span className="tabular-nums">${subtotal.toLocaleString()}</span>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); onClearCategory(category); }} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors" title="清空分類">
                            <Eraser className="h-4 w-4 md:h-5 md:w-5" />
                        </button>
                        <button 
                            onClick={() => onOpenSelection(category)} 
                            className="bg-accent text-white px-3 py-1.5 md:px-4 md:py-2 rounded-xl text-xs md:text-sm font-bold shadow-lg shadow-blue-200 hover:bg-blue-600 hover:shadow-blue-300 hover:-translate-y-0.5 transition-all active:scale-95 flex items-center gap-1"
                        >
                ...
                <div 
                    onClick={() => onOpenSelection(category)} 
                    className="flex flex-row items-center justify-between p-4 md:p-5 transition-all duration-300 gap-4 cursor-pointer hover:bg-gray-50/50 active:bg-gray-100"
                >
                    <div className="flex items-center gap-4 opacity-60 group-hover:opacity-100 transition-opacity flex-1">
                        <div className="p-2.5 rounded-xl bg-gray-100 text-gray-400 group-hover:text-black group-hover:bg-white group-hover:shadow-md transition-all">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs text-gray-400 font-medium hidden md:block group-hover:text-accent transition-colors">點擊選擇商品</span>
                        </div>
                    </div>
                    <div className="text-xs text-gray-300 flex items-center gap-1 font-medium justify-end group-hover:text-accent transition-colors">
            ...
            {hasItems && (
                <div className="flex flex-col">
                    {items.map((item) => {
                        ...
                        const ItemContent = (
                            <div className="flex flex-col md:flex-row md:items-center px-4 py-3 md:px-5 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-all relative group/item cursor-pointer">
                                <div className="flex-1 min-w-0 pr-8 md:pr-0">
                                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                                        <span className="font-bold text-sm md:text-lg text-gray-900 leading-tight group-hover/item:text-accent transition-colors">{item.name}</span>
                                        {(item.category === Category.CPU || item.category === Category.GPU) && item.specDetails?.tdp && <span className="text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-lg border border-orange-100 ml-1 whitespace-nowrap">TDP {item.specDetails.tdp}</span>}
                                        {/* Desktop Error Badge */}
                                ...
                                <div className="flex items-center justify-between mt-2 md:mt-0 md:ml-4 gap-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="md:hidden flex items-center bg-white border border-gray-200 rounded-full h-8 px-1 shadow-sm" onClick={(e) => e.stopPropagation()}>
                                            <button 
                                        ...
                                        <div className="hidden md:flex items-center bg-white border border-gray-200 rounded-lg h-9 shadow-sm">
                                            <button onClick={(e) => { e.stopPropagation(); onQuantityChange(item.id, -1); }} className="w-8 hover:bg-gray-50 h-full rounded-l-lg text-gray-600 flex items-center justify-center transition-colors" disabled={item.quantity <= 1}><Minus className="h-3 w-3" /></button>
                                            <span className="w-8 text-center text-sm font-bold text-black">{item.quantity}</span>
                                            <button onClick={(e) => { e.stopPropagation(); onQuantityChange(item.id, 1); }} className="w-8 hover:bg-gray-50 h-full rounded-r-lg text-gray-600 flex items-center justify-center transition-colors"><Plus className="h-3 w-3" /></button>
                                        </div>
