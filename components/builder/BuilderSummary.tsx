    ...
    return (
        <div className="sticky top-24 self-start bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-200 p-6 max-h-[calc(100vh-120px)] flex flex-col overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100 flex-shrink-0">
                ...
                <button onClick={onAiOpen} className="px-3 py-1.5 bg-black text-white rounded-lg text-xs font-bold hover:bg-gray-800 transition-all flex items-center gap-1.5 shadow-sm active:scale-95"><Sparkles className="h-3 w-3 text-yellow-300" /> AI 配單</button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 flex-shrink-0">
                <button onClick={() => onTemplateOpen('load')} className="px-3 py-2.5 text-sm font-bold text-gray-700 border border-gray-200 rounded-xl hover:bg-gray-50 flex items-center justify-center gap-2 transition-colors"><FolderOpen className="h-4 w-4" /> 載入</button>
                <button onClick={() => onTemplateOpen('save')} className="px-3 py-2.5 text-sm font-bold text-gray-700 border border-gray-200 rounded-xl hover:bg-gray-50 flex items-center justify-center gap-2 transition-colors"><Save className="h-4 w-4" /> 儲存</button>
                <button onClick={onShare} className="px-3 py-2.5 text-sm font-bold text-gray-700 border border-gray-200 rounded-xl hover:bg-gray-50 flex items-center justify-center gap-2 transition-colors"><Share2 className="h-4 w-4" /> 分享</button>
                <button onClick={onReset} className="px-3 py-2.5 text-sm font-bold text-red-600 border border-red-100 rounded-xl hover:bg-red-50 flex items-center justify-center gap-2 transition-colors"><RotateCcw className="h-4 w-4" /> 清空</button>
            </div>
        </div>
    );
};
